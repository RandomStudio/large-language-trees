...
export interface BrightestAndDarkest {
  brightColor: string;
  darkColor: string;
}
...
export function getColourPair(img: HTMLImageElement): BrightestAndDarkest {
  ...
  if (validPairs.length > 0) {
    ...
    return { brightColor: `rgb(255, 185, 198)`, darkColor: `rgb(117, 0, 147)` };
  }
}

function getPaletteFromImage(
  ...
  const sortedColors = Object.keys(colorMap)
    .map((key) => ({
      color: key.split("-").map((num) => parseInt(num)) as [
        number,
        number,
        number
      ],
