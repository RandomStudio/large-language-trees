<script lang="ts">
  ...
  const grassAroundPlant = (
    ...
    const startY = plantPositionData.y;
    ...
    for (let i = 0; i < numPatches; i++) {
      ...
      const offsetY = (Math.random() * plantPositionData.size) / 20;
      ...
      grassPatches.push({
        ...
        size,
        // zIndex: i === 0 && numPatches < 2 ? plantPositionData.zIndex : 1
        zIndex: plantPositionData.zIndex + 1
      });
    }
  ...
  const buildLayout = (plantsInGarden: SelectPlant[]): PlantsWithGrasses[] => {
    ...
    return plantsInGarden
      ...
      .map((plant, index) => {
        ...
        const offsetY =
          height / 2.5 + (-index * height) / plantsInGarden.length / 2;

        // Offset from centre, as determined by index
        const amplitudeX =
          index === 0
            ? 0
            : remap(index, [1, plantsInGarden.length], [width / 2, 0.0001]) *
              Math.random();
  ...
  {#each positions as { parent1, commonName, imageUrl, plantPositionData, grassPositions }}
    {#each grassPositions as grassPatch}
      <img
        ...
        style={`left: ${grassPatch.x}px; bottom: ${grassPatch.y}px; width: ${grassPatch.size}px; z-index: ${grassPatch.zIndex}; `}
    ...
    <img
      ...
      style={`left: ${plantPositionData.x}px; bottom: ${plantPositionData.y}px; height: ${plantPositionData.size}px; width: auto; z-index: ${plantPositionData.zIndex}`}
      style:transition={disableAnimations ? null : "left 2s, top 5s"}
